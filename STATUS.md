...
3. **经济通缩（食物单一栽培的机制根，量化已在）**：work 铸币小（serve+3/prepare+2）vs 房租16-23 + 吃饭~24/天 →
   全员7天内被抽干到≈0、日均只吃1.2餐 → 每段互动都在面包/饥饿轴谈判。**是调参（改 work income 或 rent），
   与第1项常量 + 第2项提示词改动一起在同一次 7 天 sim 复验**（避免多次长跑）。另配非食物关系轴（秘密/竞争/协作）
4. 低优先 backlog：约定结算判「到场slot」放宽到「兑现窗口内履行」（当前 63% 兑现率偏悲观，功能本身健康）；
   绫波丽单行复读（提示词）；2 处「厨房」幻觉地点（0.17%，机制侧已优雅报错，残余是模型乱选）
