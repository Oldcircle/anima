...
**1. 对话并行交错**
同一 tick 内两人并行决策，各自回复的是上一轮信箱的消息。导致对话看起来像两段交错的独立对话——A 问了 X，B 回的是上一轮的 Y。

原因：信箱系统是异步的，主轮和反应轮都是 Promise.all 并行执行。

修复方向（遵守第一性原理）：反应轮内串行执行而非并行，让 B 能看到 A 在本轮刚说的话再回复。不引入特殊的"对话管理器"。

**2. 单句蹦词**
talk 工具每次调用只说一句话。真人面对面说话经常一口气说好几句。当前架构下角色像发短信一样一句一句蹦。

修复方向（遵守第一性原理）：允许 talk 的 message 包含多句话（换行分隔），prompt 引导"你可以一口气说完想说的，不用一句一句地说"。不增加新工具或新机制。
...
1. **一日模拟验证**：跑 `pnpm test:sim`，验证完整一天的效果
2. **体验打磨**：前端玩家操作面板（位置/需求/金币/对话输入）
3. 可选：角色记忆可视化、关系网络图
