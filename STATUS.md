...
**下次入口（优先级被 live2 改写）**：
① **和解清账加机械核验闸**——可核验 kind（债务）账没动 → reconcile 不得清该 kind 拒绝账。
这是决裂层的前置地基（不堵，升档链永远可被一句好话拆除）
② 决裂层提案已起草待拍板：[DESIGN-rupture.md](./DESIGN-rupture.md)（心死自动+了断主动双路、
追求联动失败、回暖闸装 RelationshipManager 内部三处——已勘察）
③ 便宜三枪：结算 evidence 说话人 prompt（样本 2）、疙瘩 boiling 暖档豁免（level≥+15 不催）、
drama-verify B 对动线交集
④ 远期挂着：谎言账（对可核验事实的断言做真伪裁决，M2 三裁决同源）
⑤ 余额 ¥4.01（长弧实价 ¥0.006/调用别再指望缓存爬坡降价；够一次和解闸修复的定向短跑）
⑥ 诊断另一条「没有剪辑」仍未动
