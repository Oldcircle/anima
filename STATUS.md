...
### 结算链下半段 live 首演 PASS + 部分还款补洞 · 1254→1267 单测 · 判决 [logs/sim-drama-verify-live1-VERDICT.md](./logs/sim-drama-verify-live1-VERDICT.md)

**S1-S3 那轮已收口入仓**（分支 `drama/scene-settlement` 已提交并推远端，main 也已同步）。
本轮解决 8-22 遗留的头号问题：**升档链下半段（爽约→摊牌）在 live 里零样本**——
自然发酵要 ≥96 tick 才有一张欠条到期，300 调用只够 ~30 tick，正面跑永远够不着。

- **seeds 学会预热结算账本**（`refusals:`/`deferrals:` 两段，loadSeeds 逐字段映射 + applySeeds
  复用 recordRefusal/recordDeferral 不变式；欠条 baseline 用 `_verifiableProgress` 现算**绝不手写**；
  off 档与 `ANIMA_SETTLEMENT=0` 双闸退场）。**新剧本 `drama-verify`**：A 对种 6 tick 后到期的
  欠条（爽约推门），B 对直接种 tier2（验"开着的门有没有人走"）；无 beats 无 director（归因纪律）。
  离线彩排 `drama-verify-rehearsal.sim-mock.test.ts`（受控探针——SmartMock 乱枪会占 intent 槽，
  彩排验接线，有机性归 live 验）+ runner `drama-verify.sim.test.ts`（⛓️ drama-chain 一行判读）
- **live r1（342 调用/34 tick/¥1.85）：机制链全 PASS**——爽约、tier 直跳 2、⚡摊牌注入×5、
  摊牌戏、结算，全部 live 首演。**戏也成了**：明日香逼真嗣亮牌（"手头就7金"）、自己开价
  （"先还7剩下8说准日子"）、划界（"别再用对不起砸我"）——对照 r2 的「嗯。那您忙您的」
- ⚠️ **blocker 发现并当日修复：部分还款在物理上不存在**。`repay_debt` 浮现闸曾是
  `gold >= 全额`——谈成的"先还 7 金"没法执行，归档世界实锤欠款分文未动，asuka 的印象
  还把口头当已发生（"好歹挤出7金"）。修复：闸放宽 `gold≥1` + 工具收 `amount`（意愿/手头/
  欠额取小）+ **记忆假账红线**（没还清绝不写"还清了"）。live 复验搭下次长弧
- 结算分类器发现：判拖延时 evidence 引了**明日香自己的复述**（recency），归属反向校验
  正确拦截降未果——防线兑现了存在意义，代价是拖延续约断了一环（修法挂 VERDICT，攒样本再动）
- 意外收获：**L↔夜神月的书之争零 seeds 自燃第二条拖延弧**（「改天」×3）
- 顺带落了 r3 判决的两条处方：**失言阈值 3→2**（+标定 tripwire 单测）、**证据竞赛降噪**
  （实质关联门槛：有 trace 或运行期正典才算竞赛 + 同对每天 1 条，占坑读编年史跨 tick 稳定）
- 全部逐条变异复验（seeds 落账×2、涌现门槛×2、还款钳位——改坏必红）

### 长弧 live r2：「嘴上还钱」——和解层是升档链的阿喀琉斯之踵 · 判决 [logs/sim-drama-verify-live2-VERDICT.md](./logs/sim-drama-verify-live2-VERDICT.md)

用户充值后同日烧长弧（948 调用/154 tick/¥5.37，跑满零熔断，缓存 73.8%）。
**头号发现（FAIL 级）**：真嗣当面撒谎「15金我已经还了，就在刚才」——钱分文未动、
`repay_debt` 就在他菜单上（18 次），他选了零成本的谎；明日香信了宣布两清，
**B1 时代的和解清账凭对话就执行**（不吃机械核验），把爽约攒出的 tier 2 阶梯整个抹平。
防线不对称是根因：得手三道闸、兑现机械核验，唯独和解裸奔——谎言精确找到了唯一没设防的门。
**最毒**：明日香的印象全程记录破绽（"眼神躲闪""他哪来的钱？"），世界看见了谎，
只是没有机制消费它；对照组 L 用可颂出炉时间当场拆穿夜神月（拆谎作为涌现存在，作为机制不存在）。
**行为经济学**：在谎言被定价之前，理性 agent 不会还钱（cast-culprit「steal 0」的镜像）。
其余：⚡×13 全疙瘩路且**过热**（level +35 的朋友对还在被催架——boiling 无视暖档）；
结算判定 5/5 全对（得手两次被归属防线正确拦截）；B 对种子 tier2 没用上（动线不交，全场 4 句话）；
evidence 说话人选错复现（样本 2 次，可以动 prompt 了）。

### live2 三处方已落地（同日）：和解核验闸 + 结算说话人示例 + 疙瘩暖档豁免 · 1273 单测

- **和解不清债**：reconcile 清拒绝账前逐方向过 `_verifiableProgress`——债务类账没动 →
  该方向拒绝账与执念原地留着（立场/疙瘩/欠债人向照清）。好话说得开情绪，说不掉欠款。
  决裂层的地基已打好
- 结算 prompt 补说话人反面示例（live2 两连败的形状写进去了）；疙瘩 boiling 加暖档豁免
  （level≥+15 不催，被拒升档刻意不吃豁免）。三处逐条变异复验
- **live r3 验闸短跑（323 调用/30 tick/¥2.05）：新代码全链回归 PASS，但闸 live 无样本**——
  A 对 30 tick 零相遇（同 seeds 三跑三个样：09:15 相遇 / 06:45 撒谎 / 没遇上），
  短窗口对特定对的弧是掷骰子 → drama-verify 下版给 A 对上动线保险。
  判决 [logs/sim-drama-verify-live3-VERDICT.md](./logs/sim-drama-verify-live3-VERDICT.md)

**下次入口**：
① **决裂层待拍板**：[DESIGN-rupture.md](./DESIGN-rupture.md)（心死自动+了断主动双路、追求联动失败、
回暖闸装 RelationshipManager 内部三处——已勘察；前置的和解闸已落地）。拍板即可实现（~1 会话）
② 实现后验收弧一趟收三样：决裂判据 live 首演 + 和解闸 live 样本 + 部分还款首演。
**余额 ¥1.96 不够，先充 ≥¥8**（短跑冷启动实价 ¥0.0055-0.0065/调用、长弧 ¥0.006）
③ drama-verify 下版：A/B 对都上动线保险（作息交集或初始同地）
④ 远期挂着：谎言账（对可核验事实的断言做真伪裁决，M2 三裁决同源）；诊断「没有剪辑」仍未动
⑤ drama 分支该收口了：S1-S3 + 验收体系 + 三轮 live + 三处方全在 `drama/scene-settlement`，
main 落后一大截——用户过目后合并

## 当前状态（2026-08-22）

### 戏剧化第一轮：对话结算（S1）+ 声部减法（voice）· 1084→1182 单测全绿 · 分支 `drama/scene-settlement`（未提交）

**诊断（用户问「就算有冲突矛盾，也不像动漫游戏那样有趣，差在哪」）**：过去一个月加的全是
**世界的牙齿**（失业/追求/案件/器物/破线），没加**戏的牙齿**。翻 cast-culprit r1 转录的实证：
**每一场戏都是平局**——L 追夜神月还书追了两个游戏小时十个来回，收场「嗯。那您忙您的。」，
书没还罚款没交关系没碎，五小时后从头再来。对话是**超时过期**的不是被结果终结的；
管线四个兄弟（承诺/交易/立场/正典）**没有一个判胜负**。
平局是一切下行通路的天敌：只要"再谈一次"永远可用，绝境阶梯就永远走不到第二级
（= cast-culprit「steal 端上桌 28 次零采纳」的另一半解释）。
排第二的是**全员同一个声部**：卡里写得很不一样，模型把所有人拉平到它自己的 90 分位文笔上，
连怯懦的真嗣道歉都是工整排比。动漫的乐趣有一半来自声部对比度。

**A 层 · 对话结算**（`src/agent/settlement-extractor.ts`，对话结束管线**第五兄弟**）：
问的是**有指称**的窄问题（引擎已知每方带着什么所求进场），不是开放式"谁赢了"——
判 `得手/被拒/反将/未果`。**未果 = 平局，是合法结果，只是从此可计量**。
- 前置：`computeConversationDesire` 拆成 `computeAddressableDesire`（带 kind/id/want）+ 薄包装，
  旧调用点逐字节不动（那份 180 行逐字节断言的测试文件原样绿）。`want` **只喂结算器不进角色 prompt**
- 五道防线照 B1 同规格，其中一道是新的：**归属反向校验**——答应/回绝/反将的都是**对方**，
  evidence 必须出自 targetId 之口。另有严重度阶梯（反将无明示原话降档被拒）、每对每天 ≤1
- 落账链：拒绝账本（`narrative.refusals`，**有向**不排序，随档四件套齐）→ 执念**原地升档**
  （`upgradeObsession`，不新开 id——registerObsession 同 id 拒收，且每人 6 条 FIFO，换 id 会挤爆执念池）
  → 所求那行写硬（**追加到同一行**，不新开 prompt 槽位 = 不动缓存）→ 档位 ≥2 撞见对方，
  复用既有 argue 兜底注入摊牌 intent（措辞含「挑明」，tool-builder 靠这个词面强开 argue）
- **刻意不 setGrudge**：grudge 会冻结 registerTalk 加分、跳过 idleDecay、让 argue 兜底直接 continue——
  把人推向摊牌的同时把摊牌那条路关了。两个兜底不互踩
- 得手/和解 → 双向清账。新 kind 已进 `DEFAULT_CHEAP_KINDS`（便宜档）；日志前缀 `🧾`

**B 层 · 声部减法**（`src/agent/voice.ts`）：角色卡 `voice:` 块——字数硬顶 / 禁修辞 / 允许沉默。
**两条腿缺一不可**：prompt 侧进 system 稳定区（放在「表达准则」**之后**，才压得住那条通用的
"一两句到三四句就够了"），机械侧在 `executeAction` 唯一收口点按句边界硬截 + 违规计数。
只做 prompt 侧会重演「"可以生气"="从不"」那个老结果。
已作者：真嗣 28 / 绫波丽 24 / 明日香 40 / 月·战场原 55；**L 与鲁鲁修故意留空当对照组**——
对比度本身就是目的，全员收紧只是换一种千人一面（顺带当 A/B 对照）。

**顺手补的家族旧账**：`admin-routes.toYamlCharacter` 与 `validation.validateCharacterPayload`
**都没带 `playstyle`/`pursuit`**——用户在管理面板存一次角色卡，这两层就从 YAML 里消失
（表现和关闸一模一样，极难查）。voice 要活下去必须补这处，把邻居一起补了。

**验证**：1084→**1182 单测全绿** + tsc 干净。三档退场实测：`ANIMA_VOICE=0`/`ANIMA_SETTLEMENT=0` 全绿；
`ANIMA_BREAK_LEVEL=off` 下失败数与 main **完全相同的 7 条**（那 7 条是 main 上就有的旧账）= 本轮零新增。
**全链干跑** `settlement-chain.test.ts`（零 API）：被拒 → 账本 → 执念原地升档（仍 1 条不挤池）→
所求行写硬（只追加不改写、无换行）→ 档位 2 撞见对方 → 摊牌 intent 含「挑明」。
反证也锁了：档位只有 1 不注入。

**对抗审查（30 agent / 2.4M token，每条经独立驳斥复核）：报 25 条，确认 16 条、驳回 9 条，已全修**
- **blocker（自伤，最该记住的一条）**：那行"顺手补的旧账" `out.pursuit = card.pursuit` 把 loader
  归一化后的 **camelCase 写回 YAML**，而 `normalizePursuit` 只认 snake_case ——
  **面板存一次角色卡，追求层就被写坏在磁盘上且不可逆**（rei 整层消失，另 6 张丢
  deadlineDay/onAchieved/onFailed → 追求再也不会判失败）。修法：新增 `pursuitToYaml`
  严格逆映射 + normalizePursuit 改双接受 + validation 走 normalize。
  **教训：往 YAML 回写任何"已归一化"的对象，必须有严格互逆的 toYaml；voice 那几行做了，pursuit 那行漏了。**
- **得手是唯一会删持久状态的结果，却没有闸**：单字「嗯」甚至一个逗号即可结算成功，
  一笔勾销攒了几天的手段档位。补 `CONSENT_EXPLICIT` 明示应允词表 + `MIN_EVIDENCE_CHARS`，
  且拿不出应允原话时降档**未果**（不是降档被拒）
- **parseBlocks 焊块**：模型漏写分隔线时后一块的「结果」覆盖前一块 → 被拒被翻成得手，
  而 evidence 还是对方的回绝原话，防线③照样放行。改成**任何键重复都是块边界**
- **evidence 带发言人前缀必被丢弃**：转录渲染成 `名字：「台词」`，模型逐字抄回来最自然，
  不剥则 evidenceHits 不命中 → live 零落账的隐形杀手。`cleanEvidence` 只认转录那一种形态
  （放宽会把自带冒号的台词砍半）
- **声部截断晚于 talk 重复拦截**：拦截比的是未截断原文、记忆存的是截断后的，
  两侧口径不一致 → maxChars<60 的角色**逐字重复台词反而逃过拦截**。改成拦截前先施加
- **声部只挂 talk**：comfort/argue 的 `words`、share_secret 的 `secret` 全是后门——
  尤其 argue 正是 S1 升档链专门强开的那个。改成 `VOICE_SPEECH_ARGS` 三个出口全覆盖
- **upgradeObsession 不挪队尾**：`getActiveObsessions` 是 `slice(-limit)` 按**位置**取、
  FIFO 也按位置砍 → 刚升档的执念反而抢不到那 2 个曝光位、甚至被当最旧的删掉
- **和解清拒绝账被前置闸挡住**：结算层刻意不建 openStance 不结 grudge，纯碰壁攒的账走到
  和解分支时 `archived=0 && !hadGrudge` 直接 continue，清账那行永远够不着
- **refusals 无 TTL**：同批随档态里唯一无限期的账。补 `sweepRefusals`（5 天，接 06:00 日扫）
- **测试侧四条（都是"整层删光仍全绿"）**：真实入口 `_scheduleSettlementExtraction` 零覆盖、
  三道闸零覆盖、声部接线零覆盖、角色卡 bans/silence 零断言。全部补齐并**逐条做变异验证**
  （删掉被测代码必须变红才算数）
- 补测试时自己又逮到一个真 bug：`enforceVoice` **不幂等**——省略号让长度变成 max+1，
  而这层现在生产里会被调两次，每轮多啃一个字还重复计违规。省略号改为算进上限内

**第二轮审查（只打修复本身，23 agent）：3 条"修复"其实没修好 + 1 条新确认，已全部真修**
这一轮比第一轮值钱——**修复引入的假闸比原 bug 更危险，因为它看起来像修好了**：
- ⚠️ **我装的「得手」闸是极性盲的子串匹配**：`不能答应你` 含「答应」、`钱我不还你了` 含「还你」——
  手写 14 条回绝句 **14/14 命中词表**，连项目自带 fixture 里最硬的那句反将都能 certify 得手，
  全链实测把 tier=2 的账一次勾销。两条腿一起改：①词表加**否定感知**（前后窗查否定词，
  窗口不跨标点）→ 实测假阳性 14/14→**0/14**、假阴性 1/9 ②**得手改成非破坏性**——
  `easeRefusal` 退一档而不是清空，退到 0 才摘。**判它的是个会出错的分类器，
  不该给分类器一键抹掉几天阶梯的权力**；叙事上也更真：这回成了 ≠ 之前碰的壁一笔勾销
- ⚠️ **声部只堵了 argue 的一半**：argue 的 schema 里 `words` 可选、`reason` 才必填，
  handler 走 `words || reason` 兜底 —— 模型只填 reason 时 reason 就是那句台词。
  改成**按工具**的台词参数表（argue 收 words+reason）。`manner`/`activity` 不收（旁白与模板槽，不是嘴）
- ⚠️ **和解清账修了内层没修外层**：`_scheduleStanceExtraction` 的绕行钥匙只认 openStance，
  而结算层刻意不建 openStance —— 纯碰壁的那对在 valence 窗为空时根本进不了抽取
- 🆕 **截断切掉道歉词尾 → 疙瘩永远解不开**：中文道歉关键词几乎总在句尾，正落在硬顶外，
  实测 rei 5/5、shinji 4/5 丢关键词。新增 `voiceOriginal`（WeakMap 存截断前原文），
  道歉判定读原文——世界听见的是截断版，但"他本来想说什么"该由原文判
- 🆕 **重试去重守卫对声部角色整条失效**：比的是"截断后的旧 args vs 未截断的新 args"，
  永不相等 → 模型原样重发也会被接受。比之前先对重试参数施加同样硬顶
- 🆕 **面板保存抹掉 loader 不认识的顶层键**：`secrets`/`known_relationships`（5 张 koukou 卡，
  作者手写正文）。toYamlCharacter 是白名单式从零重建 → PUT 时合并回未知键
- **实证**：27 张卡真往返（load→validate→toYaml→parse→load）**pursuit 丢 0 / voice 丢 0 / 未知顶层键丢 0**

**有意没做（下一枪，别当遗漏）**：档位到顶（tier 3）**没有去开灰工具的门**。
STATUS 旧账写过「别调 steal 阈值，门已经开了」——本轮先证明"说这条路能走死"，
走死之后开哪扇门是下一件事。

**live 验收两跑（604 调用 / 57 tick / ≈¥1.7）：声部 PASS、结算机制 PASS、戏剧效果无样本** ·
判决 [logs/sim-s1-voice-live-VERDICT.md](./logs/sim-s1-voice-live-VERDICT.md)

- **声部 PASS 且对比度拉开了**：上限全部咬住（绫波丽最长 20/24、真嗣 28/28、明日香 40/40、
  月 52/55），全场台词中位 **52→33**，**设限组中位 28 vs 对照组 54（近 2 倍）**。
  ⚠️ 基线那列的"最长 100"是 md 渲染截断的假象（155 条里 12 条恰好 100），**只能拿中位数做对照**
- **r1 结算 0 次，真因是被「所求」层饿死了**——不是结算自己的闸有问题。所求四个源在七个陌生人的
  世界里几乎全不可达（敌对要「压力 top-1 且恰好在跟他说话」、亲密要 level≥60、约定产出 0）。
  **C2 当初「1/3 的日子留白」是对的，因为那时所求只是一行 prompt 文字；现在它同时是整个结算层的
  触发器，留白就变成了饿死。** 补**债务**源（逾期 2 天，债主向+欠债人向双向登记）后 r2 立刻点火
- **r1 还暴露我自己违了纪律**：预过滤没过那条路被写成静默 → live 零落账不可归因。已补
  `settlementSkipReason`，r2 每条跳过都能读出原因
- 🔑 **本轮最有价值的发现：判出来的「未果」是对的，缺的是分类里少了一格。**
  点火那场戏（明日香追真嗣还 15 金，十二个来回，对方还点破了"别又玩上次那套"）
  正是诊断第一条要的戏；而真嗣**既没给也没拒绝，他拖**——「答应了，然后到点没做」
  既不是得手也不是被拒，却是真实冲突里最常见的那一格，也正是原始诊断里
  「L 追夜神月还书收场『嗯，那您忙您的』」的同一件事。**拖延自带期限，所以天然可结算**：
  到点兑现=得手，到点没兑现=被拒且比第一次更重（不是没要到，是被骗了一次）。
  顺带暴露：`TIME_HINT` 认不出「打烊前」这类**事件锚定时间**，承诺没落成 appointment，拖延就蒸发

### S2 拖延（结算第四格）+ 事件锚定时间 + 追求向所求 · 1187→1228 单测全绿

**live 判决直接指出的那一格已经落地**：拖延不是终局，是**一张到期要兑现的欠条**。
- `deferrals` 随档账本（有向）：`dueTick`（固定宽限 1 游戏天，**不解析自然语言时间**——
  戏要的是"你说了你会做"）+ `baseline`（立约时的可核验进度快照）
- **兑现只认机械核验**（`_verifiableProgress`，现只有 debt=还欠多少）：核验不了的欠条到点
  **静默过期**，绝不凭 LLM 自评说他做到了（与追求层同款红线）
- `_sweepDeferrals` 每 tick 扫、**决策前跑**：兑现 → 退档（等同得手）+ 关系 +2；
  爽约 → `recordRefusal({brokenPromise:true})`，**tier = count + brokenPromises**
  （被回绝 N 次 ≠ 答应了 N 次都没做，后者该更快走到摊牌）+ 关系 −3 + 疙瘩 + LTM imp7
- **一次爽约就够到摊牌门**（tier 直接到 2）——这正是把平局变成戏的那一下
- 所求那行的爽约措辞是全系统最锋利的一句：「他答应过你 2 回了，回回到点什么都没有」；
  **欠条未到期时说的是"还没到点"不是"碰壁"**（这时候硬起来是不讲理）
- 抽取器加第四格 + `DEFER_EXPLICIT`；**模型判「得手」但原话带推后字面 → 降档拖延**
  （那不是给了，是答应了以后给）

**顺带两枪**：① `FUZZY_HOURS` 补**事件锚定时间**（打烊/收工/下班/开门/天黑/天亮）——
live 里「打烊前给你个准数」这种承诺此前整条解析不出来，约定落不了地，"答应了"说完就蒸发；
排在泛时段之前（includes 先命中先赢）② 所求补**追求向**（pursuit metric 是 relationship
且 target 就是对方）——作者预埋的方向，比"亲密度 ≥60"这种要靠时间长出来的门早得多

**变异验证**：9 处生产代码逐个改坏，其中 2 处曾"改坏仍全绿"（到期扫描的**接线**、
得手降档拖延的规则）——已补测试，现在删任一处都会红。

**对抗审查（37 agent，23 完成 / 14 撞额度限制）：确认 13 条、驳回 20 条，已全修**
- ⚠️ **blocker（同一根因三处症状）：`hostile` 分支遮蔽了 `debt`。**
  逾期欠账**自己就给 pair 压力贡献 6/天**，很容易把这一对顶过 hostile 的 40 分线——
  两条分支恰恰在 debt 要服务的那批对上撞车（真 seeds 实证：default-verify 的
  asuka:shinji 压力恰好 =40，而判据是 `>=`）。后果：① hostile **无向且不可机械核验** →
  「拖延」欠条 `baseline=undefined` → 到期静默过期，**爽约永远落不了账**，
  S2 最锋利的那一下恰恰在它唯一该生效的那一对上失效 ② `hostileDesireLine` 读的
  `debtOverdueDays` 是**无向标量** → 欠钱的那个拿到债主的怨气和债主的 want，
  被推着去向债主"要一个说法"，这个错的 want 还会喂进结算器 prompt
  ③ 压力跨线那刻 kind 翻转 → `recordRefusal` 按"换类型"把攒了几天的阶梯清零。
  修法：**debt 分支提到 hostile 之前** + hostile 的「欠账」措辞加方向闸
- ⚠️ **它还推翻了我自己写的判决归因**：`logs/sim-s1-voice-live-VERDICT.md` §2 写的
  "债务源点火"是错的，实际点火的是 hostile。判决已加勘误段
- **追求向读的是静态 `card.pursuit`**，绕过运行期 status 与 `ANIMA_PURSUIT` 闸 →
  已达成/已永久失败的追求还在催人。改读 `state.life.pursuit` + 吃总闸
- **9 处测试覆盖漏洞（全是"改坏了仍全绿"）**：`_desireLedger` 清理的**存在与顺序**
  （顺序反了整层静默死掉）、爽约的疙瘩/编年史、兑现的执念改写分支、第一次爽约的专属措辞、
  pendingDeferral 的同类匹配、事件锚定的优先级（原断言用了两个同为 20 点的词，等于没测）、
  新增时间词里三条零覆盖。全部补齐并**逐条变异复验**（六处逐个改坏，全部变红）
- 顺带清掉审查 agent 留在项目里的探针文件（它们该写进 scratchpad）

**第二轮加固（1215→1228 单测）——把那 14 条"因 agent 撞额度而从没被复核"的发现自己验完**，
8 条属实并全修，每条配回归锁 + 变异复验（逐个改坏，8/8 全部变红）：
- 🔑 **`_verifiableProgress` 的方向性错了**：欠债人向的欠条里承诺方是**债主**，
  他答应的是"宽限"——宽限没有任何机械动作可测，而旧实现回落去量**所求方自己的还款**，
  等于把功过全记在承诺方头上（欠债人不还钱 → 判债主爽约）。现在只认债主向，
  另一向不记 baseline、到期静默过期
- 🔑 **全额还清会被判爽约**：`repay_debt` 是 filter 掉整条 debt 记录，
  查不到就回落成 baseline → 真还了钱的人被判放空话。现在"查不到 = 0"
- **立约那刻账已清（baseline=0）必判爽约**（`0 < 0` 恒假）→ 只有真有欠款才记 baseline
- **`_sweepDeferrals` 没有 off 闸**（红线②）：`ANIMA_SETTLEMENT=0` 时存档里的欠条照样结算
- **`easeRefusal` 退不动档**：`tier = count + brokenPromises` 而只减 count，
  爽约过的对永远退不下来，"得手退一档"成了空话 → 爽约计数跟着退
- **`recordDeferral` 覆盖 = 免罪符**：结算每对每天限 1 次、宽限恰好也是 1 天，
  刚好够"每次快到期再答应一次"无限续期。现在未到期再答应**保留原 dueTick/baseline**
- **欠条悬着时 TTL 日扫会先把阶梯抹掉** → 立欠条刷新 `refusal.lastTick`
- **事件锚定时间词的假朋友**：「开门见山地说吧」被静默解析成早上八点的约定
  （旧行为是解析不出来 → 工具让角色重说；**自信的错时刻比解析失败更毒**）→ 加 `FALSE_FRIENDS` 剔除。
  另：`打烊` 20→18（店铺 openHours 17~22 不一，这里拿不到 workplace，取中位数少错一点，已注明是近似）
- **`DEFER_EXPLICIT` 太宽**（实测真实语料 20% 的当场应允被误降成拖延）→ 收紧词表 +
  加 `IMMEDIATE_EXPLICIT` 反向闸（「拿去，回头别再问我」是给了，不是拖）

### S3 可击穿的信念 · 1228→1254 单测全绿 —— 诊断第三条「人不会变」

**问题**：角色卡每 tick 原样重新注入，跑七天，第七天的真嗣和第一天**逐字节一样**。
故事的定义就是人物改变；此前积累的全是**外部账本**，内部自我认知一动不动——
所以真嗣可以被点破一百次「你道歉的对象不是我」，卡里还是那行「总在道歉」。

**做法**：角色卡加 `beliefs:`，每条带**机械可测**的击穿判据。
证据源正是 S1/S2 建的账本——不新开管线，只在既有落账处各 +1：
兑现/爽约在 `_sweepDeferrals`，碰壁/要到在 `_settleConversationOutcomes`；
关系与金币直接读世界（不入账，避免两份真相）。

- 六个判据指标：`kept_promises` / `broken_promises` / `refused_by_others` /
  `asks_landed` / `closest_bond` / `gold`
- **击穿不可逆**（可逆的"成长"是均值回归，不是弧）；**只换那一行字**不新增段落
- 击穿 → 长期记忆 imp9（比立场落账的 8 还高一档）+ 编年史 🫀 imp8
- 06:00 日扫，**零 LLM**——绝不问「他成长了吗」（追求层同款红线）
- 已作者五张卡：真嗣「你是个累赘」←兑现 2 次 / 绫波丽「你是可以被替换的」←最高关系 45 /
  明日香「示弱就是输」←要到 2 次 / 夜神月「这些人跟你不是一个层面」←被回绝 3 次 /
  战场原「没有人会白对你好」←兑现 2 次
- `ANIMA_BELIEFS=0` 整层退场；卡里没写 `beliefs:` 逐字节不变
- **admin 往返同步补了 `beliefs`**（这是那个家族 bug 的第四个成员：漏一处，面板存一次卡该层就消失）

**变异复验 8 处，全部变红**（含最容易漏的那种：日扫的**接线**——测试直呼私有方法时它能存活，
补了真实 06:00 tick 的回归才逮住，与 S2 同一个坑）。

⚠️ **缓存代价**：信念块在 system 稳定区，被击穿那一刻会砸一次该角色的前缀。
一局里最多发生几次，是预期成本。

**下次入口（按性价比）**：① **拉长窗口**：两跑都只到 30 tick 就撞 300 调用硬顶，拒绝账本从没攒到 tier≥2，
**升档链下半段（摊牌 intent）在 live 里一次都没演过**。⚠️ 余额已见底，第三跑前先充值
④ 诊断还剩三条没动：**人不会变**（角色卡是常量，7 天后还是第 1 天那个人；缺 `beliefs[]` 可击穿层）、
**没有剪辑**（均质流，擦杯子和失窃一个字号）、**赌注永远在关系之外**（关系会 decay 回中性，永远碎不掉）
