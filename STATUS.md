...
**下次入口**：① **充值**（余额 ¥0.44 彻底见底；短跑冷启动 ¥0.0055/调用、长跑 ¥0.004/调用）
② 长弧一趟全验（150+ tick ≈ ¥6-8）：B 对注入→上台转化、部分还款首演、拖延续约、执念/信念长跑
③ 诊断还剩两条没动：**没有剪辑**（均质流）、**赌注永远在关系之外**（关系 decay 回中性永不碎裂）——
后者与摊牌链天然接榫（摊牌的终局现在还是"又一场对话"，缺不可逆的决裂位）
