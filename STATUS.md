...
**下次入口**：① **罪案供给侧 v2**（r2 定性课题）：npc 罪案补指向性证据链——案发窗口第二目击
+informant 情报升执念（5 天保温进晨间打算）+可选静态 NPC 最小可交互；然后 r3 同弧复验
（需先充值，≈¥2）看破案/冤案终局首现——命令在 r2 VERDICT 尾部 ② M2.5 语义复述判定
③ fate beat trace 接线 ④ Bartle 玩法人格 ⑤ 前端消费（器物/正典/案件可视化）。
余额 ¥1.57，今日 live 封笔。
