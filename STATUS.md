...
### 世界接地 M0-M5：器物层 + 四通道触发 + examine + 案件收束 + 罪案供给侧 v2 · 986 单测全绿 · 规格 [PLAN-grounding.md](./PLAN-grounding.md)
...
**大戏 r2 延长弧已烧（145 tick/904 调用打顶/¥2.10）：机制层全链 PASS 收口** · 判决
[logs/sim-grounding-verify-live2-VERDICT.md](./logs/sim-grounding-verify-live2-VERDICT.md)。
r1 遗留全复验过：词面对齐修复 live PASS（风声带器物真名；rei 真去 examine 了自己的收银匣）、
tamper 误用未复现、12 小时窗排除时间假设。**指控 0 定性=证据供给不足非机制缺陷**：全 log
"赵三"零对话提及——撬痕匿名、赃物袋无人可见、informant 情报沉底记忆没上对话、静态 NPC
不社交——没人有理由指向任何人，指控 0 是理性。意外好戏：rei 丢 58 金币后"我饿了可我没钱…
我可以不吃了"——**后果层真实，罪案不破也有重量**。

**罪案供给侧 v2 已落地（2026-08-17，979→986 单测全绿，live 待烧）** · 规格 PLAN-grounding「M5」。
r2 的定性课题（指控 0 = 证据供给不足，非机制缺陷）三条通路全接：
- **案发窗口第二目击**（`crime-supply.ts supplyNpcCrimeLeads`）：投放同时给情报者之外的**第二人**
  一条"看见赵三贴着发现地点门口来回转了两趟"的观察——地点用**真名**（r1 词面教训），
  两条线索文本不同、落在不同人身上，受害者不拿线索（她只有延迟发现那条路）
- **线索执念保温**：情报/目击各挂 5 天执念（`relatedId=caseId`），沉底记忆变晨间打算与此刻区的
  主动线索；结案随 `clearObsessionsRelatedTo` 清账，不留悬着的疑心
- **静态 NPC 最小可交互**（`replyAsStaticNpc`，零 LLM）：三档递进（敷衍/不耐烦/打发走），
  per「角色×NPC」计数随档；**只有案子已公开且真凶正是他**，第 3 次试探起才可能撞见**失言**
  （否认一件你压根没提过的事）→ 挂执念。试探不到回应的人不是嫌疑人是布景
- **信息隔离是本期红线**：立案时案子还没公开 → 线索文本与执念绝不提失窃；失言的公开门同理
  （引擎不泄露没人知道的罪）。单测锁住这一条
...
**提示词追踪面板（同日，1002→1021 单测）**——项目本身是 agent 系统，"这一次到底喂了什么进去"
此前只能翻控制台或开 `ANIMA_PROMPT_DUMP` 落盘手工 diff，全是离线事后的。现在在线：
- `src/providers/prompt-trace.ts` 环形缓冲（默认 200 条，`ANIMA_PROMPT_TRACE_KEEP=0` 整层关闭，
  单条文本超 24KB 截断）记每次调用的完整 system/工具表/messages/响应/usage/缓存命中；
  provider 在 `chat()` 外包一层，**成功失败两路都记**（白丢的调用最需要被看见）
- **杀手级是前缀断点**：拿本次的工具表+system 与**上一次同「类型×角色×模型」**的**逐字节**比
  （Buffer 比不是字符串比——中文一字 3 字节，按字符报位置对不上缓存实际偏移），
  报出首个分歧字节位置 + 落在哪一段 + 前后文对照。命中率掉了的归因从"落盘 diff 两个 JSON"
  变成面板上一行字。首次调用不算断点，换角色不比（否则满屏假断点）
- `GET /api/prompts`（列表/统计/详情/清空）+ 管理面板「提示词」页：统计卡、按 kind 分桶表
  （调用/token/命中率/断点数/均耗时/失败）、调用流水、详情抽屉六个页签
- 浏览器实测：注入一次"时间行回流 system"的典型事故 → 面板准确报「前缀在第 918 字节断开（共 943 B），
  分歧落在 system」并给出前后文

**世界编年史（同日，1021→1034 单测）**——用户反馈"个人监控这些 agent 的行为太累了"。
问题不是缺面板，是**信号淹在几千行 emoji 日志里**。编年史让世界自己报：
- `src/world/chronicle.ts`：条目带重要性 1-10、类型、角色、**机械判据**；**id 由内容决定 = 幂等**
  （探测器每 tick 全量重跑，不维护游标——游标读档会错位）；随档（world_state `chronicle` 键）
- `src/world/emergence.ts` **六个涌现探测器**，直接对齐 PLAN-grounding §3 验收信号：
  ①证据竞赛（同日 ≥2 人查同一器物）②共享虚构成真（正典被**非首述者**复述——M2.5 的 restate
  裁决在这里第二次兑现）③篡改痕迹被他人发现（信号②）④流言到 3 人⑤执念挂满 3 天⑥关系翻向（死区 ±5）。
  **涌现条目必须带 evidence**，说不清判据的宁可不报
- 重大事件在既有日志同处补 record：案件发现/冷案/accuse 三路/tamper/命运/饿倒/还钱/kira 应验/
  NPC 投放/失言。tamper 的 trace id 补上动手的人（探测器要排除篡改者自己）
- `GET /api/chronicle` + 管理面板「编年史」页：按天分组、每天一条头条、默认只看 ≥7 分、
  可切"只看涌现"、点条目展开机械判据
- 浏览器实测：六类探测器全部触发并给出判据（如「trace tamper_140_light_botched@140 之后，
  l_lawliet 于 tick 152 examine 过 library.shelves」）

**存档调度补漏（同日，1034→1045 单测）**——用户要"关掉了也能接着上次进度"。
查下来存档系统本身没问题（SQLite 单事务、启动自动读档、按剧本归属档案），
破的是**触发时机**，三个都在长跑里真会咬人：
- **只认 SIGINT**：本项目长跑纪律是 screen/nohup 脱管，被杀时来的是 **SIGTERM**，
  关终端是 SIGHUP，崩溃是 uncaughtException——一个都不走 SIGINT。「r1 被后台超时杀」
  那次丢的就是这条路。现在 `save-manager.ts` 全覆盖 + 崩溃兜底
- **96 tick 才自动存**（一整个游戏日 ≈24 现实分钟）→ 默认降到 24 tick，`ANIMA_AUTOSAVE_TICKS` 可调、=0 关
- **覆盖式无备份** → 覆盖前轮转 `.bak`
- 另加：命名快照（`data/snapshots/`，"跑 r3 之前留一手"不用手动 cp）、`GET/POST /api/save`、
  设置页存档区块（上次存档/大小/次数 + 立即存档 + 建快照）
- **手动存档排到 tick 边界**：tick 内部有 await，中途存会存出"一半角色已决策"的世界，
  读档后那个 tick 重跑、已行动的角色会再动一次。API 只登记意图，tick 末尾落地
- **端到端实测**（零 API）：跑起来 → `SIGTERM` 自杀 → 新进程读档，tick/金币/编年史全接上

**半日烟测 PASS（2026-08-20，285 调用/27 tick/≈¥0.5）** · 判决
[logs/sim-grounding-smoke-VERDICT.md](./logs/sim-grounding-smoke-VERDICT.md)。
目的不是验剧情，是验本轮新代码在真 API 下没搞坏底盘——三项 PASS：
① **provider 包装层**（提示词追踪包在 chat() 外，每次调用都过）285 次零失败零熔断
② **玩法人格没抖前缀**：冷启动爬到 60.6%@275 调用且仍在涨，与 r1（64.1%）同带
③ **涌现探测器真 live 首演**：27 tick 落 4 条全为"证据竞赛"，密度不刷屏；
   L↔夜神月连撞借阅台账+书架（宿敌在图书馆互相盯着查同一批东西，无机制安排）
无样本项（时间尺度预期内，非失败）：浮现层/NPC试探（罪案要 48 tick 预热，本轮只到 tick 51）、
restate 裁决、tamper/命运/accuse。观察项：3 次 impression 截断是**老账非回归**（r2 同项 27 次，
且 impression 自建 system 不吃玩法人格块）。

**存档系统做成"像游戏那样"（同日，1045→1050 单测）**——用户起跑 r3 前的四问，
查下来只有一问是支持的，其余三个是真缺口，补完再跑：

| 问题 | 补前 | 补后 |
|---|---|---|
| 跑起来能自由涌现吗 | ✅ 本来就能 | — |
| 能查跑的记录吗 | ⚠️ 只有 markdown 战报 | ✅ **世界也归档**了 |
| 能打开记录续跑吗 | ❌ runner 完全不存档 | ✅ `pnpm dev --load <档>` |
| 多个记录 / 新开档 | ❌ 快照建了读不回 | ✅ 命名存档 + 四类档案清单 |

- **`run-archive.ts`**：sim runner 的 finally 里归档到 `data/runs/<label>-<时间戳>.db`
  （跑完/达顶/熔断都走）。**这是最大的一块**——否则一趟 40-60 分钟 ¥2 的长跑，
  编年史/器物/正典/案件跑完全部蒸发，判读只能人肉翻 log，更不可能续跑
- **命名存档**：`data/saves/<名字>.db`；CLI `--load <名字|路径>` / `--new <名字>`；
  面板「另存为并切换」（不动世界，只改以后往哪写，旧档原样留着）
- **档案清单** `GET /api/save/slots` + 面板：主档/命名档/快照/长跑归档四类，
  各带剧本与 tick（`peekSave` 探针只读元信息不装世界），直接给出打开命令。
  **面板不做运行中热加载**——切世界要重建对话追踪/导演/脉冲一堆内存态，风险大于收益
- 端到端实测（零 API）：跑到 tick99 归档 → 打开确认编年史/案件完整 → 续跑到 105 →
  另存为两个命名档并存，清单正确分类

**r3 大戏已烧（902 调用/155 tick/tick24→178/≈¥7.9）：供给侧 v2 落地成功，指控仍 0 但卡点精确定位** ·
判决 [logs/sim-grounding-verify-r3-VERDICT.md](./logs/sim-grounding-verify-r3-VERDICT.md) ·
**世界归档** `data/runs/grounding-verify-deepseek-chat-20260820-160051.db`（`pnpm dev --load <它>` 可续跑）

逐格对 r2：案件公开 ❌→**✅立案@72/公开@73** · 线索执念 2→**3** · **NPC试探 0→2（浮现层兑现，
赵三第一次被当成人搭话）** · 缓存 64.1%→**69.8%**（长程稳定，玩法人格没抖前缀）· 指控 0→0。

**指控为什么还是 0——三个可指认的原因，r2 那句"没人有理由指向任何人"已不成立**：三条线索执念
活着且两条直接点名赵三。真卡点是①**差一次试探**（`NPC_PROBE_SLIP_AT=3`，跑到 2 次撞硬顶；
失言是全局唯一能把"这人不对劲"抬成实证的机关）②accuse 破案门是人赃俱获，而赃物在 NPC 背包里
无人可见③两天弧从公开到收尾只剩 ~26 游戏小时。

**本轮最该正视的问题：涌现 11 条全是同一类**（证据竞赛），另五个探测器全 0；且同对同天刷屏
（鲁鲁修↔L 一天 4 条）。47 次 examine 出 11 条竞赛=触发率偏高。修法在判决里。
...
**失业机制已落地（1065→1074 单测）——回答"为什么不如动漫好看"的第一枪**。
诊断：这个世界**没有任何不可逆的终局**（grep 实证：无死亡/离镇/永久失业；关系 decay 回中性、
需求会刷新）。**每一天都可以重来 → 等待永远是最优解** → 这才是 cast-culprit r1
「steal 端上桌 28 次零采纳」的真正原因：不偷的代价只是继续饿着，而明天需求刷新。
`aspiration` 也只是注入 prompt 的一行字符串，无进度/障碍/期限/失败态——
**没有人在追求什么，所有人都只是在维持**（PLAN-grounding 附「任务化」那条正是这个病）。
落地：`world/employment.ts` 连续缺勤 2 天捎话警告+挂执念、3 天辞退（workplace 清空/收入归零/
worker 工具消失/风声传开/编年史）。**先警告后辞退**是纪律。`ANIMA_JOB_LOSS=0` 整层关。
干跑实证全链接通，且 SmartMock 的 asuka **自己**连缺三天被辞——机制会自燃。
⚠️ **风险观察项**：干跑里七人全被辞（SmartMock 不会主动上工，是 mock 缺陷非机制缺陷），
但真 agent 若出勤率也低，会塌成"全镇失业"的熵增而不是戏。
**下次 live 必须盯出勤率**；`ABSENCE_FIRE_DAYS` 是常量，好调。

**追求已落地（1074→1084 单测）——"不像动漫"诊断的另一半**。
失业解决"没有输"，追求解决"**没有想要**"：`aspiration` 此前只是一行注入 prompt 的字符串，
needs 全是恒稳态。**需求是回归性的（推你回平衡点），目标是方向性的**。
`world/pursuit.ts`：角色卡 `pursuit:` 声明可测目标——进度绑真实状态（金币/技能/关系/持有物，
不是 LLM 自评）+ 期限 + **到期即永久失败**。七人各作者一条（四种口径都用上）。
干跑实证全链接通（注入此刻区→进度→结算→编年史）。
⚠️ **校准是这层的命门**：首版把技能目标定成 6，查实测才发现技能 **+0.05/次工作**
（到 6 要 120 次），且 asuka 卡里根本没有 barista 技能——**必不可达**。
已按真实增长率重校（技能→3、light 金币 200→160、lelouch 关系 45→40）。
**改目标前先查增长率**：必成和必败一样不是戏。
...
1. **深翻**：third 档 system prompt 顶块也切作者/预测框架（`break-config.ts thirdPersonIdentityHead` +
   prompt-builder 换头，first 档逐字节回归）——「成为他」→「写他/预测他」，消除顶块与决策层的打架。
   决策+对话两路共用同一顶块，一处生效。
2. **私有通道（七反转④）**：决策指令加两层动机行契约`【表面】…｜【真心】…`（带「与表面一致」诚实豁免，
   防旧「疑惑」槽位式自激）。`motive-channel.ts` 解析 → `AgentTickResult.motive` → WS tick payload +
   `🎭 [motive]` 日志（观看者可见）；**本人记忆只回流表面层**（truncate 前替换）——真心层不进任何角色
   prompt，自欺不因记忆回流坍塌（七反转③）。
3. **门禁数据**：两层动机 129 条（≈79% 决策，07-08 半翻时自欺标记仅 36%）且伤口驱动（shinji「怕被
   赶走」/senjougahara「手动着就不用想没人值得见」）；泄漏 0；负 valence 8:3；缓存 63.8% 无回归。

**下次入口 = 7 天联合复验（用户暂缓，等放行再跑）**：命令与统计口径备在判决文件里（nohup 脱管 +
strong+third，~55 分钟 ~2700 调用）。看点=分层比例走势（79% 是否过度生产）+ 表面层长程自我叙事 +
上轮 5 条遗留观察项。前端消费 `motive` 字段（web 面板/Godot 真心层展示）未做，等 7 天验过再立项。

### 决策视角实验：第三人称作者预测解锁「自欺/两层动机」（已提交 a4943fc）· 判决见 [logs/POV-experiment-20260708-VERDICT.md](./logs/POV-experiment-20260708-VERDICT.md)
