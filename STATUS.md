...
1. **打通下行通路（最高优先，第 2+3 节同根）**：让已存在的对话摩擦能转成负向 tick / grudge。最小改动二选一——
   **① talk 收益递减**（`+1` 双击是通胀源，随当日已聊次数 + 当前 level 衰减，关系要维护非累积）或 **② idle 衰减**（无互动的对每天小 -Δ）。落地后 `负向和=0` 才不再是假信号，第 2 节的闹掰-和好弧线才可能出现。**需再跑一次 7 天 sim 验证**
2. **打破食物单一栽培**：引非食物关系轴（秘密/竞争/协作/物交），降食物关键词密度
3. 清理：绫波丽单行复读循环、2 处「厨房」幻觉地点、结算判定放宽到「兑现窗口内履行」
