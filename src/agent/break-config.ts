...
疑惑：（对方让你单纯好奇/看不透的地方，和疙瘩不是一回事，没有写"无"。用你自己的口吻写具体的疑惑——禁止「TA是不是在试探什么」这类侦探腔套话，你不是侦探；疑惑可以是对方的来历、生活、心事、某句反常的话、某个对不上的细节。）`;
}

// ───────────────────────── C4/C7 活人感质感（DESIGN-revival §3）─────────────────────────

/**
 * C4 — 疑惑槽节流档位：未解 ≥ 返回值次才把疑惑注回 prompt（impressions.formatForPrompt /
 * prompt-builder curiosities 消费）。切断"读到自己上轮的疑惑→本轮接着试探"的反馈回路
 * （审计：'试探' 印象区 19 次/console 31 次，跨 5+ 角色）。off = 1（不节流，治愈系基线逐字节回归）。
 */
export function unresolvedThrottleMinCount(): number {
  return on() ? 2 : 1;
}

/**
 * C7 — 开场白反模板 nudge（决策路径，有人在场时注入此刻区）。
 * 审计实锤：r4 全程 15 条"早上好"开场/295 对话，三组对话同构开场。off = ""。
 */
export function openerAntiTemplateNudge(): string {
  if (!on()) return "";
  return `开口别用模板寒暄：「早上好」「你也早起？」「今天天气不错」这类开场全镇人人都在说，说了等于没说。要开口，就从此刻具体的东西切入——对方正在做的事、你心里挂着/好奇/膈应的事、上次没说完的话；没什么真想说的，就别为了寒暄硬开口。`;
}

/**
 * C7 — 食物话题对冲：餐饮类地点的对话别被菜单占满（审计：食物话题 46% 第一引力井）。
 * 正负混合话题供给——好奇的/膈应的/没了结的/约好的/高兴的都比菜单有分量，不是全换成催账。
 * 非餐饮地点返回 ""；off = ""（治愈系基线逐字节回归）。地点在对话内不变，不抖前缀缓存。
 */
export function foodTopicHedge(locationName: string): string {
  if (!on()) return "";
  if (!/面包|咖啡|酒|餐|食堂|饭/.test(locationName)) return "";
