...
- 不要所有角色都用同一种"温柔礼貌"的语气。每个人说话的方式应该截然不同${harmonyReflexBans()}`);
  ...
  // B2 letter_arrival 剧本门控豁免行：manifest 声明 world_events: [letter_arrival] 的剧本里
  // 镇外的信/物件可以真实送达（实体物品入包）——不豁免则硬事件与物理法则打架。
  // 静态 per-run（scenario 加载时定死），不抖前缀缓存；off 档不加（治愈系基线逐字节回归）。
  const letterException =
    isWorldEventEnabled("letter_arrival") && getBreakLevel() !== "off"
      ? "。唯一例外：镇外寄到你手里的信或物件是真实存在的（它会出现在你的记忆里、随身物品里），可以读、可以谈论——但寄信的人本人依然不会出现，也见不到"
      : "";
  parts.push(`\n## 真实边界（这是世界的物理，不是风格建议）
...
- 你过去认识的人（家人、故人、旧识）都不在这个镇上，也联系不上。可以想念、可以被回忆硌着，但他们不会出现，也别安排跟他们见面或说"他来过"${letterException}`);
